...
> **制定日期**: 2026-03-07 | **已完成**: #18-23 | **总题数**: 246（唯一题号；含新增高频缺口、Intervals、Design / Data Structure）
> **策略**: 专题突破，由易到难，间隔复习
...
| 2 | 217 | Contains Duplicate | 🟢 哈希集合判重，基础但高频 |
| 3 | 66 | Plus One | 🟢 简单数组操作 + 进位思维 |
| 4 | 26 | Remove Duplicates | 🟢 快慢指针在数组中的基础用法 |
| 5 | 27 | Remove Element | 🟢 与 #26 同类型，巩固双指针覆盖 |
| 6 | 169 | Majority Element | 🟢 Boyer-Moore 投票法，面试高频 |
| 7 | 118 | Pascal's Triangle | 🟢 逐行构建，DP 思维启蒙 |
| 8 | 119 | Pascal's Triangle II | 🟢 #118 的进阶，学会滚动数组优化 |
| 9 | 189 | Rotate Array | 🟡 三次翻转法，经典原地操作 |
...
| 18 | 73 | Set Matrix Zeroes | 🟡 原地标记技巧 |
| 19 | 36 | Valid Sudoku | 🟡 行/列/宫哈希，综合哈希应用 |
| 20 | 31 | Next Permutation | 🟡 找规律题，降序→交换→翻转 |
| 21 | 41 | First Missing Positive | 🔴 原地哈希终极应用，理解索引映射 |
...
| 3 | 242 | Valid Anagram | 🟢 字符计数基础，#49 的前置题 |
| 4 | 125 | Valid Palindrome | 🟢 双指针在字符串中的应用 |
| 5 | 67 | Add Binary | 🟢 模拟加法进位，与 #66(Plus One) 呼应 |
...
| 13 | 43 | Multiply Strings | 🟡 竖式乘法逐位，#67 进阶版 |
| 14 | 8 | String to Integer (atoi) | 🟡 状态机/边界处理，面试常考 |
| 15 | 65 | Valid Number（选做） | 🔴 完整有限状态机实现，低 ROI |
| 16 | 68 | Text Justification（选做） | 🔴 贪心模拟排版，综合模拟能力 |

### 1.3 链表（16 + 3✅ + 1模板 = 20 题）
...
| 147 | Insertion Sort List（选做） | 🟡 | 链表插入排序 | ⬜ |
...
| 8 | 234 | Palindrome Linked List | 🟢 快慢指针 + 反转后半段的组合练习 |
| 9 | 19 | Remove Nth Node From End | 🟡 快慢指针变体 ✅ |
| 10 | 24 | Swap Nodes in Pairs | 🟡 递归/哑节点，依赖 #206 思路 |
| 11 | 92 | Reverse Linked List II | 🟡 区间反转，#206 的直接升级版 |
| 12 | 82 | Remove Duplicates II | 🟡 哑节点 + 跳过，#83 的进阶 |
| 13 | 86 | Partition List | 🟡 双链表分割合并 |
| 14 | 61 | Rotate List | 🟡 成环 + 断开 |
| 15 | 143 | Reorder List | 🟡 综合题：找中点 + 反转(#206) + 合并(#21) |
| 16 | 138 | Copy List with Random Pointer | 🟡 哈希/交织复制，拓宽思路 |
| 17 | 147 | Insertion Sort List（选做） | 🟡 链表排序入门 |
| 18 | 148 | Sort List | 🟡 归并排序链表，#147 后做 |
| 19 | 25 | Reverse Nodes in k-Group | 🔴 分组反转，依赖 #206 + #24 |
| 20 | 23 | Merge k Sorted Lists | 🔴 多路归并，依赖 #21 ✅ |

### 1.4 栈 & 队列（8 + 1✅ + 1模板 = 10 题）
...
| 155 | Min Stack | 🟡 | 辅助栈同步 | ⬜ |
| 394 | Decode String | 🟡 | 栈解析嵌套结构 | ⬜ |
| 678 | Valid Parenthesis String | 🟡 | 贪心上下界 / 栈 | ⬜ |
...
| 5 | 394 | Decode String | 🟡 嵌套字符串解析，高频栈应用 |
| 6 | 678 | Valid Parenthesis String | 🟡 括号合法性 + 贪心上下界，高频变体 |
| 7 | **739** | **Daily Temperatures** 🏷️ | 🟡 **单调栈核心模板**，后续题依赖 |
| 8 | 84 | Largest Rectangle in Histogram | 🔴 单调递增栈经典，依赖 #739 思路 |
| 9 | 42 | Trapping Rain Water | 🔴 单调栈/双指针，与 #84 思路相通 |
| 10 | 32 | Longest Valid Parentheses | 🔴 栈/DP，#20 的终极升级版 |

---

## 📅 第二阶段：核心算法范式（第 4-6 周）

### 2.1 双指针 & 滑动窗口（13 + 1✅ + 1模板 = 15 题）
...
| 15 | 30 | Substring with Concat of All Words（选做） | 🔴 多起点滑窗，#76 思路 + 更复杂约束 |
...
| 顺序 | # | 题目 | 理由 |
|:----:|---|------|------|
| 1 | 912 | Sort an Array | 🟡 排序基本功，练归并/快排/堆排 |
| 2 | 75 | Sort Colors | 🟡 荷兰旗三路分区，排序基础 |
| 3 | 56 | Merge Intervals | 🟡 排序后合并，区间问题入门 |
| 4 | 57 | Insert Interval | 🟡 #56 的变体，区间插入合并 |
| 5 | 179 | Largest Number | 🟡 自定义排序规则，面试常考 |
| 6 | 164 | Maximum Gap（选做） | 🔴 桶排序 O(n)，理解非比较排序 |

### 2.4 Top K / 快速选择 / 堆（4 题）

| # | 题目 | 难度 | 关键点 | 状态 |
|---|------|------|--------|------|
| 215 | Kth Largest Element in an Array | 🟡 | 快速选择 / 最小堆 | ⬜ |
| 347 | Top K Frequent Elements | 🟡 | 哈希表 + 堆 / 桶排序 | ⬜ |
| 973 | K Closest Points to Origin | 🟡 | 堆 / 快速选择 | ⬜ |
| 692 | Top K Frequent Words | 🟡 | 堆 + 自定义排序 | ⬜ |

**📋 推荐刷题顺序：**

> 第 k 大 → 频率 Top K → 距离 Top K → 字符串 Top K

| 顺序 | # | 题目 | 理由 |
|:----:|---|------|------|
| 1 | 215 | Kth Largest Element in an Array | 🟡 先理解第 k 大，排序/堆/快速选择三种做法 |
| 2 | 347 | Top K Frequent Elements | 🟡 在 #215 基础上加频率统计 |
| 3 | 973 | K Closest Points to Origin | 🟡 把 Top K 扩展到按距离比较 |
| 4 | 692 | Top K Frequent Words | 🟡 频率 + 字典序，自定义规则更完整 |
...
| 15 | 37 | Sudoku Solver（选做） | 🔴 约束回溯终极题 |

---

## 📅 第三阶段：树 & 图 & 并查集（第 7-9 周）
...
| 19 | 199 | Right Side View | 🟡 BFS/DFS 取最右，层序应用 |
| 20 | 116 | Next Right Pointers | 🟡 层次连接 O(1) 空间 |
| 21 | 117 | Next Right Pointers II | 🟡 非完美树版本，#116 升级 |
| 22 | 236 | Lowest Common Ancestor of a Binary Tree | 🟡 分叉点判断，面试高频 |
| 23 | 105 | Construct from Pre+Inorder | 🟡 分治 + 哈希定位 |
| 24 | 106 | Construct from In+Postorder | 🟡 分治，与 #105 对称 |
| 25 | 114 | Flatten to Linked List | 🟡 前序展开 |
| 26 | 297 | Serialize and Deserialize Binary Tree | 🔴 树结构编码/还原，面试高频设计题 |
| 27 | 124 | Maximum Path Sum | 🔴 后序 + 全局最大值，树的终极题 |

### 3.2 二叉搜索树 BST（7 + 2模板 = 9 题）

| # | 题目 | 难度 | 关键点 | 状态 |
|---|------|------|--------|------|
| 95 | Unique Binary Search Trees II（选做） | 🟡 | 递归生成所有 BST | ⬜ |
| 96 | Unique Binary Search Trees | 🟡 | 卡特兰数 DP | ⬜ |
| 98 | Validate Binary Search Tree | 🟡 | 中序/上下界 | ⬜ |
| 99 | Recover Binary Search Tree（选做） | 🟡 | 中序找两个错位 | ⬜ |
...
| 5 | 99 | Recover BST（选做） | 🟡 中序找两个错位，#98 的进阶 |
...
| 399 | Evaluate Division | 🟡 | 带权图 DFS/BFS | ⬜ |
| 417 | Pacific Atlantic Water Flow | 🟡 | 反向多源 DFS/BFS | ⬜ |
| 743 | Network Delay Time | 🟡 | Dijkstra 最短路 | ⬜ |
| 785 | Is Graph Bipartite? | 🟡 | 二分图染色 | ⬜ |
| 994 | Rotting Oranges | 🟡 | 多源 BFS 分层扩散 | ⬜ |
...
> 网格 DFS 入门 → 图遍历 → 拓扑排序 → Hard BFS

| 顺序 | # | 题目 | 理由 |
|:----:|---|------|------|
| 1 | 200 | Number of Islands | 🟡 网格 DFS/BFS 入门，图论第一题 |
| 2 | **695** | **Max Area of Island** 🏷️ | 🟡 DFS 计面积，#200 的自然扩展 |
| 3 | 994 | Rotting Oranges | 🟡 多源 BFS，面试高频扩散模型 |
| 4 | 130 | Surrounded Regions | 🟡 边界 DFS 标记，#200 变体 |
| 5 | 417 | Pacific Atlantic Water Flow | 🟡 反向可达性，多源 DFS/BFS 高频题 |
| 6 | 133 | Clone Graph | 🟡 BFS/DFS + 哈希，图的复制 |
| 7 | 399 | Evaluate Division | 🟡 把关系建成带权图，业务建模高频 |
| 8 | 785 | Is Graph Bipartite? | 🟡 图染色，补齐二分图模型 |
| 9 | **207** | **Course Schedule** 🏷️ | 🟡 拓扑排序 BFS，检测环 |
| 10 | **210** | **Course Schedule II** 🏷️ | 🟡 拓扑输出序列，#207 的进阶 |
...
| # | 题目 | 难度 | 关键点 | 状态 |
|---|------|------|--------|------|
| **547** | **Number of Provinces** 🏷️模板 | 🟡 | 连通分量计数 | ⬜ |
| **684** | **Redundant Connection** 🏷️模板 | 🟡 | 无向图判环 / 找多余边 | ⬜ |
| **1319** | **Number of Operations to Make Network Connected** 🏷️模板 | 🟡 | 边数下界 + 连通分量 | ⬜ |
| **721** | **Accounts Merge** 🏷️模板 | 🟡 | 实体合并 / 邮箱归属 | ⬜ |
| **990** | **Satisfiability of Equality Equations** 🏷️模板 | 🟡 | 等式合并 + 不等式检查 | ⬜ |

**📋 推荐刷题顺序：**

> 连通分量 → 边表判环 → 连接网络 → 实体合并 → 约束一致性

| 顺序 | # | 题目 | 理由 |
|:----:|---|------|------|
| 1 | **547** | **Number of Provinces** 🏷️ | 🟡 并查集入门，先掌握 find/union/count |
| 2 | **684** | **Redundant Connection** 🏷️ | 🟡 用 union 判断无向图里哪条边制造了环 |
| 3 | **1319** | **Make Network Connected** 🏷️ | 🟡 先判断边数是否足够，再用连通分量算操作数 |
| 4 | **721** | **Accounts Merge** 🏷️ | 🟡 把字符串实体映射成节点，练习并查集建模 |
| 5 | **990** | **Satisfiability of Equations** 🏷️ | 🟡 先合并等式，再检查不等式冲突 |
...
| 9 | 187 | Repeated DNA Sequences（选做） | 🟡 哈希/位压缩 |
...
| 7 | 981 | Time Based Key-Value Store | 🟡 哈希表 + 二分，设计和算法结合 |
| 8 | 146 | LRU Cache | 🟡 哈希表 + 双向链表，设计题核心 |
